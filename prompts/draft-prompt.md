...
3. If the harness supports subagents and the research questions are independent, dispatch bounded read-only workers for knowledge-base evidence, domain evidence, or model/interface evidence. Give them explicit questions and require sources, applicability, disagreements, and limits. They must not edit the prompt target or draft competing final prompts. If delegation is unavailable or would add no independent evidence, research directly.
