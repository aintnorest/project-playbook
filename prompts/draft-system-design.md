...
5. Make each shared interface and failure boundary concrete enough to constrain its slices. Label introduced interfaces `[PROPOSED]` and interfaces verified in supplied or inspected source `[EXISTS]`. If an interface cannot be verified and the distinction matters, verify it or raise it as an open decision tagged `[NEEDS YOUR CALL]`; if it does not matter, leave it unlabeled. A rule belongs here only when it crosses slices or defines their boundary.
