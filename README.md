...
3. Choose an action in the [prompt library](prompts/README.md). It covers evidence-grounded prompt creation and independent prompt evaluation, document creation and convergence, targeted TypeScript/Rust/Tauri code reviews, handoff, and implementation-plan execution through isolated subagents; each task has an agent source and a chat-ready version.
...
The [evidence-grounded prompt drafting decision](decisions/0006-evidence-grounded-prompt-drafting.md) records the local-first research, single-owner, model-neutral, and scoped-adaptation foundations. The [prompt responsibility decision](decisions/0007-separate-prompt-construction-and-evaluation.md) records why prompt creation and prompt evaluation are separate tasks. The [selective prompt-review evidence decision](decisions/0008-selective-research-in-prompt-evaluation.md) records how evaluation retrieves only applicable research, accounts for developer-supplied behavioral history, and traces compiled findings to editable owners before shared-source changes.
...
- Replace `prompts/sdd-review.md` and the unprefixed document-review entry points with `review-doc-product-vision.md`, `review-doc-prd.md`, `review-doc-system-design.md`, `review-doc-technical-design.md`, and `review-doc-implementation-plan.md`; rename `review-dry.md` to `review-doc-dry.md`. Use `prompts/README.md` to find all authoring and lifecycle tasks, or paste the matching file from `prompts/chat/` without collecting separate guides.
...
- Rename `review-typescript.md`, `review-rust.md`, and `review-tauri.md` to `review-code-typescript.md`, `review-code-rust.md`, and `review-code-tauri.md`. They remain separate read-only correctness and maintainability reviews, share the catalog's code review contract, preserve polyglot scope boundaries, and have generated chat-ready copies.
...
- Use `prompts/draft-prompt.md` to create or materially revise one evidence-grounded prompt from a use case; it does not execute or review its output. Use `prompts/review-prompt.md` separately to inspect the generated artifact users run and exercise behavior when the target runtime is available. Prompt review maps effective instructions back to the task source, shared guidance, publisher, and model/interface layer; a shared-source finding names its consumer scope and requires cross-prompt impact review before correction. It also selectively retrieves relevant knowledge-base evidence and accounts for supplied prior failures, strengths, raw responses, and comparison results without treating an older artifact as proof of current behavior. Repository agents prefer local Project Playbook and knowledge-base checkouts, falling back to `https://github.com/aintnorest/project-playbook` and `https://github.com/aintnorest/knowledge-base` only when the corresponding local repository is unavailable.
