...
- [Google's review standard](https://google.github.io/eng-practices/review/reviewer/standard.html) and [review criteria](https://google.github.io/eng-practices/review/reviewer/looking-for.html) informed review for code health rather than perfection. A maintainability finding can show present change coupling or comprehension cost without inventing a runtime failure, but preferences and hypothetical scale do not become defects.
- [OpenAI's prompting guidance](https://developers.openai.com/api/docs/guides/prompt-engineering) and [Anthropic's prompting guidance](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/claude-prompting-best-practices) informed explicit roles, scoped source context, evidence-grounded output, and representative behavior checks. The prompts use no provider-specific tool names, model settings, or performance claims for GPT-6-Astra or Fable 5.1.
- [TypeScript's type-assertion guidance](https://www.typescriptlang.org/docs/handbook/2/everyday-types.html#type-assertions), [function design guidance](https://www.typescriptlang.org/docs/handbook/2/functions.html), and [module theory](https://www.typescriptlang.org/docs/handbook/modules/theory.html) informed the distinction between static declarations, runtime contracts, useful types, and actual host behavior. The review does not prescribe annotations, immutability, strictness migrations, or schema libraries without a concrete reason.
- [Rust API Guidelines](https://rust-lang.github.io/api-guidelines/checklist.html), [Cargo features](https://doc.rust-lang.org/cargo/reference/features.html), [Tokio's mutex guidance](https://docs.rs/tokio/latest/tokio/sync/struct.Mutex.html), and [Serde representations](https://serde.rs/enum-representations.html) informed caller burden, supported configurations, ownership, and boundary contracts. Idioms are conditional evidence rather than bans on clones, standard mutexes, panic assertions, or dynamic dispatch.
- [Tauri capabilities](https://v2.tauri.app/security/capabilities/), [calling Rust](https://v2.tauri.app/develop/calling-rust/), [calling the frontend](https://v2.tauri.app/develop/calling-frontend/), and [v1-to-v2 migration guidance](https://v2.tauri.app/start/migrate/from-tauri-1/) informed version-sensitive IPC, lifecycle, and authority checks. The framework review covers its integration code and configuration regardless of frontend language, not generic Rust/TypeScript style; local app-command defaults are not plugin permissions or remote-access guarantees.

Scope is an explicit cost boundary: change reviews exclude unrelated existing debt, snapshot reviews do not invent a baseline, and other languages supply only the connected contract needed to assess the target. Generated code may supply evidence, but corrections belong at its authoritative source; missing counterparts limit a check rather than prove a mismatch.

### Verification and limits

Twelve text-only completion trials using the `default` configuration exercised all three generated prompts. Supported findings covered a TypeScript/host runtime representation mismatch, duplicated quota-policy ownership without a current runtime failure, Rust I/O-error loss, a Tauri listener disposal race, and custom Rust filesystem access bypassing the stated window/path policy.

The sound-code trials returned zero supported findings for a required TypeScript copy, matching Rust/foreign field names, a short standard mutex scope with intentional panic/shutdown policy, a public local Tauri app command without a per-command capability, and a healthy v1/plain-JavaScript integration. Additional cases excluded unchanged debt from a diff review and kept an unavailable foreign contract out of the Rust findings.

The publisher freshness check and local Markdown-link checks passed; no publisher code changed and no permanent test files or temporary repository fixtures were added. These are text-only review calibration results, not execution of the reviewed applications, a full-repository review, or separate reliability evaluations of GPT-6-Astra and Fable 5.1.
